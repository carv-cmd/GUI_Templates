...
ttk.Label(sub1, text="-- Hypothesis Calculator -- ").grid(column=1, row=0)
...
for child in range(1, field_val.get()):
    entry_block = ttk.Entry(sub2, width=7, textvariable=f'Var:{child}')
    entry_block.grid(column=3, row=2, sticky=(W, E), padx=3)
    user_entry = IntVar()
